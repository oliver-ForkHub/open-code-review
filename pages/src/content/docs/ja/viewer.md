...
このページの長い一覧——**File breakdown**、**Files Reviewed**、**Session Tasks**、
**Conversations**——は 1 ページあたり 20 件です。1 ページに収まる間、ページャーは非表示のままです。
...
各グループは 1 ページあたり 20 件の指摘を表示し、1 ページに収まるときはページャーは非表示です。
...
フィルタバーのチップで重大度やカテゴリに絞り込めます。コメントは 1 ページに
20 件表示され、1 ページしかないときはページャーは非表示です。重大度やカテゴリの
フィルタを変えると 1 ページ目に戻ります。所見のマーク、**Hide marked** の切り替え、
マークのクリアは、そのページが残っている限り現在のページを保ちます。
